...
function RelatedCardItem({ project, contentMt15 }: { project: ProjectCard; contentMt15: boolean }) {
    const to = `/portfolio/${project.slug}`;
    return (
        <div className="alt-portfolio-item at-hover-item mb-30">
            ...
            >
                ...
                <div className="alt-portfolio-btn">
                    <div className="content">
                        ...
                        <h2 className="fw-400 fz-font-3xl text-white mb-0 mt-20">{project.tagline ?? project.title}</h2>
                        <p className="text-white fz-font-md mb-0 mt-10 text-truncate-2 des">{project.tagline ?? ""}</p>
                    </div>
                </div>
            </Link>
            <div className={`alt-portfolio-content d-flex justify-content-between align-items-center ${contentMt15 ? "mt-15" : ""}`.trim()}>
                <h5 className="alt-portfolio-title mb-0">
                    <Link to={to} className="common-underline">
                        {project.title}
                    </Link>
                </h5>
...
export default function Section2({ related = [] }: Props) {
                ...
                <div className="row mt-30">
                    {related.map((project, idx) => (
                        <div key={project.slug} className="col-lg-4 col-md-6">
                            <RelatedCardItem project={project} contentMt15={idx < 2} />
                        </div>
                    ))}
                </div>
            </div>
